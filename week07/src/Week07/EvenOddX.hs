...
mkGameValidator game bsZero' bsOne' dat red ctx =
    ...
    case (dat, red) of
       ...
       (GameDatum bs (Just c), Reveal nonce) -> 
           ...
           traceIfFalse "missedDeadline"                (to (gRevealDeadline game) `contains` txInfoValidRange info)       &&
           traceIfFalse "wrong stake"                   (lovelaces (txOutValue ownInput) == (2 * gStake game))             &&
           traceIfFalse "NFT must go to first player"   nftToFirst

       -- Second player does not respond
    ...
    case m of
        ...
        Just (oref, o, dat) -> case dat of
            ...
            GameDatum _ (Just c') | c' == c -> do
                logInfo @String "second player play payed and lost"
                let lookups = Constraints.unspentOutputs (Map.singleton oref o) <>
                              Constraints.otherScript (gameValidator game)
...
secondGame :: SecondParams -> Contract s w Text ()
secondGame sp = do
    pkh <- pubKeyHash <$> Contract.ownPubKey
    let game = Game
            { gFirst          = spFirst sp
